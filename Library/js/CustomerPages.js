jQuery(function ($) {
    ...
    function handleSingleProductGallery(scrollContainer, product) {
        ...
        if ( productImages.length > 1 && !product.find('.product-image-dots').length) {
            ...
            if ( !product.parents('.clb-popup-product').length ) {
                var data = 'data-scroll="#scroll-product"';
            }
    ...
    function handleSubscribeModal() {
        switch (datassoh.subscribe_popup_type) {
            case 'time':
                setTimeout(function () {
                    SubscribeModal();
                }, +datassoh.subscribe_popup_var * 1000);
                break;
            case 'scroll':
                var ckeck = true;
                $(window).on('scroll', function (e) {
                    ...
                    if (ckeck && scrollPercentRounded > datassoh.subscribe_popup_var) {
                        SubscribeModal();
                        ckeck = false;
                    }
                });
    ...
    if (datassoh.subscribe_popup_enable && !getCookie('subscribeCookie')) {
    ...
    $(window).on('load', function () {
        ...
        $('body').on('click', '.notification-bar .clb-close, .notification-bar .notification-btn a', function (e) {
            ...
            setCookie('notification', 'enabled', +datassoh.notification_expires);
