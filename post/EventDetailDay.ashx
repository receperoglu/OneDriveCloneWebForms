    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                RandevuListesi.Parameters.AddWithValue("@tarih",tarih);
                SqlDataReader ok;
                ...
                while (ok.Read())
                {
                    ...
                    olaylar += "<div class='col-md-12 personeldetail' style='background:" + ok["renk"].ToString() + "'>  <div class='col-md-10'> <span data-pk='" + ok["personelid"].ToString() + "' class='PersDetail'> <span title='Yetkili' alt='Yetkili' class='fa fa-user'></span> " + ok["personel"].ToString() + "</span><br> <span title='Firma' alt='Firma' data-pk='" + ok["firmaid"].ToString() + "' class='CorpDetail'><span class='fa fa-building'></span>    " + ok["firma"].ToString() + "</span><br><span title='Tarih - Saat' alt='Tarih - Saat' class='fa fa-calendar'></span> " + ok["tarih"].ToString() + " - " + saat + "</div><div  class='col-md-2 randevuiptal'>  <span data-pk='" + ok["id"].ToString() + "' class='randevuiptalislem fa fa-eject'></span>    </div> </div>";
                }
